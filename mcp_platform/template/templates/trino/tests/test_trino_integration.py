...
class TestTrinoTemplateStructure:
    ...
    def test_template_structure(self, template_dir):
        ...
        for old_file in old_files:
            assert not (
                template_dir / old_file
            ).exists(), f"Old file should be removed: {old_file}"
...
class TestTrinoServerIntegration:
    ...
    def test_authentication_configuration(self, mock_create_engine):
        ...
        basic_config = {**self.test_config, "trino_password": "secret123"}
    ...
    def test_write_mode_with_warning(self, mock_create_engine):
        ...
        write_config = {**self.test_config, "trino_allow_write_queries": True}

        with patch("builtins.print") as mock_print:
            TrinoMCPServer(config_dict=write_config, skip_validation=True)
    ...
    def test_query_limits_enforcement(self, mock_create_engine):
        ...
        limited_config = {**self.test_config, "trino_max_results": 3}
    ...
    def test_sqlalchemy_trino_integration(self, mock_create_engine):
        ...
        TrinoMCPServer(config_dict=self.test_config, skip_validation=True)
    ...
    def test_template_examples_accuracy(self):
        ...
        basic_example = next(
            (ex for ex in cli_examples if "trino_password" in ex), None
        )
        assert basic_example is not None

        # Should include OAuth example
        oauth_example = next(
            (ex for ex in cli_examples if "oauth_enabled=true" in ex), None
        )
    ...
    def test_logging_configuration(self, mock_create_engine):
        ...
        log_config = {**self.test_config, "log_level": "debug"}
        template_path = os.path.join(os.path.dirname(__file__), "..", "template.json")
