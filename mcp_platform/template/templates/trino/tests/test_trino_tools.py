...
class TestTrinoServerTools:
    ...
    def create_test_server(self, config_override=None):
        ...
        with patch('server.create_engine') as mock_engine:
            ...
            mock_conn = Mock()
            mock_result = Mock()
            mock_result.fetchall.return_value = [("test_catalog",), ("another_catalog",)]
            mock_result.fetchone.return_value = ("test_value",)
            mock_result.__iter__ = Mock(return_value=iter([{"col1": "val1", "col2": "val2"}]))
    ...
    def test_list_schemas_tool(self):
        ...
        # Mock the SQL result
        mock_conn.execute.return_value.fetchall.return_value = [
            ("schema1",), ("schema2",), ("information_schema",)
        ]
    ...
    def test_execute_query_max_results_limit(self):
        ...
        mock_rows = [
            Mock(_mapping={"id": i, "value": f"val{i}"}) 
            for i in range(5)
        ]
        mock_result = Mock()
    ...
    def test_get_query_status_tool(self):
        ...
        mock_conn.execute.return_value.fetchone.return_value = [
            "query_123", "SELECT * FROM table", "FINISHED", "2023-01-01 10:00:00"
        ]
